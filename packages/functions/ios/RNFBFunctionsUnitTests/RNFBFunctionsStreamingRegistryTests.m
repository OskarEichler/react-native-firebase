...
- (void)testShouldForwardEvent_done_takesAndForwards {
  ...
  BOOL forward = [self.registry shouldForwardEvent:@{@"done" : @YES} listenerId:@2];
...
- (void)testShouldForwardEvent_chunkWhenPresent_forwards {
  ...
  BOOL forward = [self.registry shouldForwardEvent:@{@"done" : @NO} listenerId:@3];
...
- (void)testShouldForwardEvent_chunkAfterTake_skips {
  BOOL forward = [self.registry shouldForwardEvent:@{@"done" : @NO} listenerId:@4];
  XCTAssertFalse(forward);
}

- (void)testShouldForwardEvent_doneAfterTake_skips {
  BOOL forward = [self.registry shouldForwardEvent:@{@"done" : @YES} listenerId:@5];
  XCTAssertFalse(forward);
}
