...
public class RNFBMessagingNotificationRegistryTest {
  ...
  public void putReplacing_whenFree_stores() {
    RNFBMessagingNotificationRegistry<String> registry = new RNFBMessagingNotificationRegistry<>();
    registry.putReplacing("msg-1", "payload");
    assertSame("payload", registry.get("msg-1"));
  }
  ...
  public void putReplacing_whenOccupied_replacesLastWins() throws Exception {
    RNFBMessagingNotificationRegistry<String> registry = new RNFBMessagingNotificationRegistry<>();
    registry.put("msg-1", "first");
    registry.putReplacing("msg-1", "second");
  ...
  public void putReplacing_sameIdFromTwoThreads_lastWins() throws Exception {
    ...
    AtomicReference<String> prevA = new AtomicReference<>();
    AtomicReference<String> prevB = new AtomicReference<>();
    ...
    Thread t1 =
        new Thread(
            () -> {
              try {
                start.await();
                prevA.set(registry.putReplacing("msg-1", "second"));
    ...
    Thread t2 =
        new Thread(
            () -> {
              try {
                start.await();
                prevB.set(registry.putReplacing("msg-1", "third"));
              } catch (InterruptedException e) {
                Thread.currentThread().interrupt();
              } finally {
                done.countDown();
    ...
    String value = registry.get("msg-1");
    assertTrue("second".equals(value) || "third".equals(value));
    String returnedFirst = "first".equals(prevA.get()) ? prevA.get() : prevB.get();
